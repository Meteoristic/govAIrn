        ...
        >
            ...
            <div className="relative h-[--mark-slider-height]">
                <div ref={sliderRef} className="absolute inset-0">
                    <div className="absolute top-1/2 -translate-y-1/2 w-full h-[--mark-slider-track-height] bg-indigo/10 rounded-sm overflow-hidden">
                        {/* Progress overlay */}
                        <div
                            className={cn(
                                "absolute top-0 h-full z-[1] bg-indigo"
                            )}
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        "bg-phosphor dark:bg-phosphor/90"
                                    )}
                    ...
                    {sliderValues.includes(0) && (
                        ...
                        >
                            <div className="h-3 w-[--mark-slider-marker-width] bg-cyan -translate-x-[calc(var(--mark-slider-marker-width)/2)]" />
                        </div>
                    )}
                    ...
                    >
                        {/* Triangle */}
                        <div className={cn(
                            "w-0 h-0 border-[5px] border-transparent border-b-indigo mt-2",
                            isOutOfBounds && "border-b-indigo/20"
                        )} />
                        {/* Square */}
                        <div className={cn(
                            "w-[10px] h-[10px]",
                            isOutOfBounds ? "bg-indigo/20" : "bg-indigo"
                        )} />
                        {/* Text */}
                        <div className="absolute top-[22px] left-1/2 -translate-x-1/2 whitespace-nowrap">
                            <span className={cn(
                                "text-xs font-medium text-phosphor",
                                isOutOfBounds && "opacity-75"
                            )}>
                                {isOutOfBounds 
...
>(({ className, ...props }, ref) => (
    <label
        ref={ref}
        className={cn("text-xs font-medium text-silver/70", className)}
        {...props}
    />
))
...
>(({ className, prefix, suffix, ...props }, ref) => {
    ...
    return (
        <div 
            className="group inline-flex items-center bg-indigo/5 rounded px-0.5 focus-within:ring-1 focus-within:ring-indigo cursor-text w-20"
            onClick={handleContainerClick}
        >
            {prefix && <span className="text-xs text-phosphor/75 select-none shrink-0">{prefix}</span>}
            <input
                ref={(node) => {
                ...
                className={cn(
                    ...
                    "tabular-nums text-phosphor",
                    className
                )}
                {...props}
            />
            {suffix && <span className="text-xs text-phosphor/75 select-none shrink-0">{suffix}</span>}
        </div>
    )
})
